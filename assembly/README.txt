The FORSYTH-2200 Assembler and Simulator
===============

To aid in testing your processor, we have provided an *assembler* and
a *simulator* for the LC-2200f architecture. The assembler supports
...
the assembler. The LC-2200f assembler definition is included.

Included Files
-----------

* `assembler.py`: the main assembler program
* `lc898.py`: the LC-2200f assembler definition
* `lc898-sim.py`: the LC-2200f simulator program
...
Example usage to assemble `test.s`:
    ...
    python assembler.py -i lc898 --sym test.s
...
To output assembled code in hexadecimal (for use with *CircuitSim*):
    ...
    python assembler.py -i lc898 --hex test.s
...
In addition to the syntax described in the LC-2200f ISA reference,
